...
public class MapPanel extends JPanel {
	...
	private void renderPathModel(Graphics2D g2d, PathModel pm) {
		...
		if(pm.intersections.size() < 2)
			return;
		
		double x[] = new double[pm.intersections.size()+1];
		double y[] = new double[pm.intersections.size()+1];
		...
		x[0] = start.x;
		y[0] = start.y;
		x[pm.intersections.size()] = end.x;
		y[pm.intersections.size()] = end.y;
		
		for(int i = 1; i < pm.intersections.size(); ++i) {
		...
		for(int i = 1; i < x.length; ++i) {
			Point2D.Double p1 = new Point2D.Double(x[i-1], y[i-1]);
			Point2D.Double p2 = new Point2D.Double(x[i], y[i]);
		...
		Ellipse2D.Double eel = new Ellipse2D.Double(end.x-w/2, end.y-h/2, w, h);
		g2d.draw(eel);
	}
