...
public class MapModel extends Model {
	...
	public Intersection lookupIntersectionForAirplane(AircraftModel am) {
		for(int i = children.size() - 1; i >= 0; --i) {
			Model m = (Model)children.get(i);
			
			if(m instanceof ShapeModel) {
				Area a = areaFromShapeModel((ShapeModel)m);
				
				if(a.contains(am.location)) {
					ArrayList isects = lookupIntersectionsInShape((ShapeModel)m);
					if(isects.size() > 0)
						return (Intersection)isects.get(0);
				}
			}
		}
	...
	public void loadIntersectionsFile(String isectFile) {
		...
		while(index < lines.size()) {
			...
			is.index = intersections.size();
			is.fileIndex = Integer.parseInt(idx);
