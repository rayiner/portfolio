...
public class Main {
	...
	public static void appendPilotFrameText(String name, String clearance) {
		Iterator itr = pilotFrames.iterator();
		while(itr.hasNext()) {
			PilotFrame pf = (PilotFrame)itr.next();
			if(pf.aircraftName == name)
				pf.appendClearance(clearance);
		}
	}
	...
	public static void main(String[] args) {
		...
		while(itr.hasNext()) {
			...
			if(m instanceof AircraftModel) {
				PilotFrame pf = new PilotFrame(m.name);
				pf.setVisible(true);
				pilotFrames.add(pf);
			}
		}
